...
export default function GenerarCertificado(props) {
  ...
  const printDocument = () => {
    ...
    html2canvas(input).then((canvas) => {
      ...
      pdf.save("certificado_abcloud.pdf");
    });
  };
  ...
  if (
    ...
    !curso.inscriptionStudent.ending
  ) {
    props.history.push("/");
    return null;
  }
    ...
    <div id="divToPrint" className={classes.root}>
      ...
      <div className={classes.textoSecundario}>
        <Typography variant="h6" align="justify">
          ...
          <b>"{curso.course.title}"</b> en la plataforma de AB Cloud, iniciado
          el {formatoFechaCertificado(curso.inscriptionStudent.createdAt)} y
          terminado el{" "}
      ...
      <div className={classes.codigo}>
        <Typography variant="button">
          N° {curso.inscriptionStudent.numCertificate}
        </Typography>
      </div>
    </div>
  );
